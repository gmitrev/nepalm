...
(function() {
  $(document).on("ajax:send", "form#new_comment", function(event, data, status, error) {
    $("textarea#comment_body").val("");
    return true;
  }).on("ajax:success", "form#new_comment", function(event, data, status, error) {
  ...
  $(document).on("ajax:send", ".delete-comment", function(e) {
    var container;
  ...
  $(document).on("ajax:send", ".subscribe-button, .unsubscribe-button", function(e) {
    $("a.subscribe-button").toggle();
    return $("a.unsubscribe-button").toggle();
  });
...
(function() {
  ...
  $(document).on("ajax:send", "form#new_task", function(event, data, status, error) {
    $("input#task_name").val("");
    return true;
  }).on("ajax:success", "form#new_task", function(event, data, status, error) {
    $("ul#task_list").append(data);
    $("div#no-tasks-msg").hide();
    return $(".best_in_place").best_in_place();
  });

  $(document).on("ajax:send", ".complete-task", function(e) {
    var checkbox, container, task_id;
    container = $(this).parents("li.task");
    task_id = container.data("task-id");
    checkbox = $(this).children("i");
    container.toggleClass("task-done");
    return checkbox.toggleClass("fa-square-o fa-check-square");
  });

  $(document).on("ajax:send", ".delete-task", function(e) {
    var container;
    container = $(this).parents("li.task");
    return container.fadeOut(function() {
      container.remove();
      if ($("li.task").size() === 0) {
        return $("div#no-tasks-msg").show();
      }
    });
  });

  $(document).on("click", "#show-completed-tasks", function(e) {
    $("#task_list_completed").slideToggle();
    if ($(this).hasClass("less")) {
      $(this).removeClass("less");
      $(this).children(".action-show").show();
      return $(this).children(".action-hide").hide();
    } else {
      $(this).addClass("less");
      $(this).children(".action-show").hide();
      return $(this).children(".action-hide").show();
    }
  });
